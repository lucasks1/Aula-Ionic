<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $sql = $con->query("insert into produtos (descricao, quantidade, valor) values ('".$data->descricao."','".$data->quantidade."','".$data->valor."')");   
        if($sql){
            $data->codigo = $con->insert_codigo;
    ...
    if($_SERVER['REQUEST_METHOD'] === 'PUT'){
        if(isset($_GET['codigo'])){
            ...
            $codigo = $con->real_escape_string($_GET['codigo']);
            $data = json_decode(file_get_contents("php://input"));
            $sql = $con->query("update produto set descricao = '".$data->descricao."', quantidade = '".$data->quantidade."', valor = '".$data->valor." where codigo = '$codigo'");
            if($sql){
    ...
    if($_SERVER['REQUEST_METHOD'] === 'DELETE'){
        if(isset($_GET['codigo'])){
            $codigo = $con->real_escape_string($_GET['codigo']);
            $sql = $con->query("delete from produtos where codigo = '$codigo'");
